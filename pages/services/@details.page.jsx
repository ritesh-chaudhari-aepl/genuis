...
import FeaturesCard from "../../src/components/cards/FeaturesCard";
import Breadcrumb from "../../src/breadcrumbs/Breadcrumbs";
import BannerPart from "../../src/components/services/service-details/BannerPart";
...
import BuyNowSection from "../../src/components/services/service-details/BuyNowSection";
import Newsletter from "../../src/components/containers/Newsletter";
import { options } from "../../src/data/nav";
...
//   //   },
//   // ]},
//   // canonical: `https://thegeniustutors.com/${slug}`,
//   // url: `https://thegeniustutors.com/${slug}`,
// };
...
function Page() {
  ...
  const formatRouteParam = (slug) => {
    return slug.replace(/-/g, " ").replace(/%20/g, " ");
  };
  const filteredFaq = faq.filter((item) => item.label === slug);
  // console.log(filteredFaq, "filterdFAQ");
  const filteredOptions = options.filter(
    (service) =>
      // service.label === formatRouteParam(slug)
      service.label === formatRouteParam(slug)
  );
  ...
  const bannerTitle =
    currentServiceOption?.bannerTitle || `${formatRouteParam(slug)}`; // Assuming you have an aboutImage property in your options
  const bannerDesc =
  ...
  const aboutTitle = currentServiceOption?.aboutTitle || (
    <>
      Genius Tutors <span className="capitalize">{formatRouteParam(slug)}</span>{" "}
      - One place for best quality{" "}
      <span className="capitalize">{formatRouteParam(slug)}</span>
    </>
  );
  const howWorksTitle = currentServiceOption?.howWorksTitle || (
    <>How {formatRouteParam(slug)} works</>
  );
    ...
    <div>
      <div className="justify-between bg-white flex flex-col pb-10">
        ...
        <div className="service-about-section mx-auto py-16 max-w-screen-xl px-6">
          <div className="grid md:grid-cols-2 gap-5">
            <div className="content-part grid gap-3">
              ...
              {options
                .filter((service) => service.label === formatRouteParam(slug))
                .map((service, index) => {
        ...
        <div className="service-why-section mx-auto py-12 max-w-screen-xl px-6">
          <div className="grid md:grid-cols-2 gap-5">
            <div className="content-part grid gap-3">
              <h4 className="text-2xl  font-semibold text-black">{whyTitle}</h4>
              ...
              {options
                ...
                .map((service, index) => {
                  ...
                  return (
                    <React.Fragment key={index}>
                      {service.serviceWhySectionPara.map((para, paraIndex) => (
                        <p
                          key={paraIndex}
                          className="text-justify  leading-7 tracking-wide"
                        >
        ...
        <div className="feature-section relative mx-auto py-16 max-w-screen-xl px-6">
          <h4 className="text-4xl  text-center font-semibold text-black capitalize">
            ...
            <span className=""> {formatRouteParam(slug)} </span>
          </h4>
